...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function generateGalleryItems() {
        const galleryHTML = (0, _galleryItems.galleryItems).map((item)=>`<a class="gallery__item" href="${item.original}">
            <img class="gallery__image" src="${item.preview}" alt="${item.description}" />
          </a>`).join("");
        gallery.innerHTML = galleryHTML;
        instance = new (0, _simplelightboxDefault.default)(gallery.querySelectorAll("a"), {
            showCounter: true,
            captionDelay: 250,
            captionsData: "alt"
        });
    ...
    function imageSelect(event) {
        ...
        instance.open();
        instance.slideToIndex(Array.from(gallery.querySelectorAll("a")).indexOf(clickedElement.parentNode));
    }
